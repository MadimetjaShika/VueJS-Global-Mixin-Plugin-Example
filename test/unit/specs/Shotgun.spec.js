import Vue from 'vue';
import Shotgun from '@/components/Shotgun/Shotgun';
import App from '@/App';

const triggerActionText = '12 Gauge Shotshell Fired, BAM!';

describe('Shotgun.vue', () => {
  it('inits correct default data', () => {
    expect(typeof Shotgun.data).toBe('function');
    const defaultData = Shotgun.data();
    expect(defaultData.triggerAction).toBe(triggerActionText);
  });

  it('should render correct contents', () => {
    const Constructor = Vue.extend(Shotgun);
    const vm = new Constructor().$mount();

    expect(vm.$el.querySelector('.title h1').textContent)
      .to.equal('Shotgun');
  });

  it('trigger pull alerts correct text', () => {
    const Constructor = Vue.extend(App);
    const vm = new Constructor().$mount();

    const button = Shotgun.$el.querySelector('button');
    button.dispatchEvent(new window.Event('click'));
    // Shotgun._watcher.run();

    expect(vm.$el.querySelector('.notification-text p').textContent).to.contain(triggerActionText);
  });
});
